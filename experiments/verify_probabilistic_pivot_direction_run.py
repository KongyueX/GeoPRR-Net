"""Verify a completed probabilistic direction run before frozen evaluation."""
...
def _state_health(state: dict[str, torch.Tensor]) -> dict[str, Any]:
    ...
    for name, tensor in state.items():
        ...
        for prefix in required_prefixes:
            required_prefixes[prefix] |= name.startswith(prefix + ".")
    ...
    missing = [name for name, present in required_prefixes.items() if not present]
    if missing:
        raise ValueError(f"checkpoint misses model heads: {missing}")
    ...
    *,
    ...
    for index, record in enumerate(history, start=1):
        ...
        finite_values = [
            ...
            train_metrics.get("angular_nll_loss"),
            ...
            validation_metrics.get("mean_angle_std_degrees"),
        ...
        key = (
            ...
            float(validation_metrics["angular_calibration_nll"]),
            float(validation_metrics["pivot_mean_error_fraction"]),
            index,
        )
        if best_key is None or key[:3] < best_key[:3]:
            best_key = key
    ...
    if best_epoch != best_key[3] or not math.isclose(
    ...
    if best.get("protocol") != PROBABILISTIC_PIVOT_DIRECTION_PROTOCOL:
    ...
    return {
        ...
        "best_validation_angular_calibration_nll": best_key[1],
        "best_validation_pivot_error_fraction": best_key[2],
        "epochs": expected_epochs,
        "train_samples": len(train),
        "validation_samples": len(validation),
