"""Create paper-facing JSON and Markdown tables for uncertainty routing."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "artifacts/runs/uncertainty_router_syncg/uncertainty_router_comparison.json"
        ),
    )
...
def main() -> None:
    ...
    payload = {
        "schema_version": 1,
        "protocol": "uncertainty_router_paper_summary_v1",
        "training": {
        ...
        "feature_ablation": {
            **ablation,
            "summary": str(ablation_path),
            "summary_sha256": sha256_file(ablation_path),
        },
        "evaluations": evaluations,
        "interpretation": {
            "syncg_all_point_estimates_improve_base": all(
                evaluations[name]["paired_comparisons"]["router_vs_base_mask"]
                ["delta_nmae"] < 0.0
            ...
            ["paired_comparisons"]["router_vs_base_mask"]["delta_nmae"] < 0.0,
            "combined_point_estimate_improves_quality_v1": evaluations[
                "combined_severe"
            ]["paired_comparisons"]["router_vs_quality_router_v1"]
            ["delta_nmae"] < 0.0,
        },
    }
    _atomic_write(
    ...
    lines = [
        "# Frozen probabilistic uncertainty-router comparison",
        "",
        "The router and threshold are fitted from SyncG/train grouped nested OOF rows only.",
        "",
        "| Condition | Base | Prob. vector | Quality v1 | Soft fusion | Uncertainty router | VDN | Router Acc@2% | Delta vs base (95% CI) |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for condition in CONDITIONS:
        result = evaluations[condition]
        metrics = result["metrics"]
        comparison = result["paired_comparisons"]["router_vs_base_mask"]
        interval = comparison["group_bootstrap_95ci"]
        interval_text = f"[{interval[0]:+.4f}, {interval[1]:+.4f}]"
        lines.append(
            f"| {labels[condition]} | {metrics['base_mask']['nmae']:.4f} | "
            f"{metrics['probabilistic_vector']['nmae']:.4f} | "
            f"{metrics['quality_router_v1']['nmae']:.4f} | "
            f"{metrics['uncertainty_soft_fusion']['nmae']:.4f} | "
            f"{metrics['uncertainty_router']['nmae']:.4f} | "
            f"{metrics['vdn']['nmae']:.4f} | "
            f"{metrics['uncertainty_router']['acc_2pct']:.4f} | "
            f"{comparison['delta_nmae']:+.4f} {interval_text} |"
        )
    lines.extend(
        [
            "",
            "## Train-only nested-OOF router feature ablation",
            "",
            "| Features | NMAE | Acc@2% | Switches | Positive / negative |",
            "|---|---:|---:|---:|---:|",
        ]
    )
    for name, values in ablation["results"].items():
        lines.append(
            f"| {name} | {values['nmae']:.4f} | {values['acc_2pct']:.4f} | "
            f"{values['switches']} | {values['positive_transfers']} / "
            f"{values['negative_transfers']} |"
        )
