...
This repository contains the model implementation, experiment entry points,
VDN comparison code, and focused tests for **GeoPRR-Net**. The manuscript
source, figures, and aggregate figure data are maintained separately in
[GeoPRR-Net-Paper](https://github.com/KongyueX/GeoPRR-Net-Paper).
...
The matched VDN experiment is complete. GeoPRR-Net reduces pooled NMAE by 39.8%
relative to three independently retrained terminal VDN checkpoints; the paired
scene-bootstrap GeoPRR-Net-minus-VDN difference is -0.6607%FS (95% CI -0.8251
to -0.4854). VDN receives annotation-derived pivot and ordered scale endpoints
for offline direction-to-progress conversion, so the manuscript treats this as
an annotation-assisted component comparison rather than an automatic end-to-end
system ranking.
...
The completed formal run evaluates every terminal epoch-200 checkpoint on the
same 1,558-image, six-condition roster (9,348 rows per seed). Its publication
aggregate is stored in
[`figures/vdn_supplement.csv`](https://github.com/KongyueX/GeoPRR-Net-Paper/blob/main/figures/vdn_supplement.csv),
and the associated three-panel figure is generated by the common manuscript
figure script.

## Manuscript and figures

The *Electronics* LaTeX source is maintained in the companion
[GeoPRR-Net-Paper repository](https://github.com/KongyueX/GeoPRR-Net-Paper).
Clone it alongside this code repository, then rebuild the complete figure set
from its included aggregate CSV files with:
