# Historical FastAPI and V5 → OCR → GARC Guide

> [!NOTE]
> This document preserves the repository's pre-ReMSTNet service guide and the
> earlier fully automatic V5 → OCR → GARC research pipeline. The root
> [README](README.md) now documents the final ReMSTNet-v3 paper release. Unless
> stated otherwise, commands below are run from the repository root.
