...
> 截至 2026-07-24，六组控制退化、RPM 外测、VDN 三种子公平重训、概率方向消融、进度校准和
> 最终选择路由均已完成。最终方法在 clean / severe perspective / severe combined / RPM
> 上的 NMAE 为 `0.0890 / 0.1454 / 0.2184 / 0.2842`，同协议重训 VDN 为
> `0.1482±0.0002 / 0.1712±0.0004 / 0.2380±0.0024 / 0.3754±0.0153`。七个条件的
> NMAE 配对区间均低于 0，但 severe
...
论文主对比保留五行同任务方法。VDN 的三次公平重训与 21 组冻结评测已经完成；HARR 官方权重也已
...
14. 同一 RTX 4060、AMP FP16、batch=1 下的 Ours/VDN/Original Transformer 参数量、
    FLOPs、峰值显存、神经栈延迟和总活动栈延迟。
...
上述 14 项现已完成。投稿前仍需完成的关键确认性实验是：冻结当前提交和权重后，在完全未参与
开发的现场留出集上一次性运行最终方法与基线；当前 SyncG、RPM 和 Pointer-10K 结果继续按
支持性/开发性证据表述。
