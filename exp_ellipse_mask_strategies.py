...
对比3种方案:
1. HSV当前方案 (baseline)
2. HSV + Canny边缘融合 (只在HSV区域内做边缘检测)
3. Canny边缘直接拟合 (激进，有抓到外圈金属壳的风险)

生成对比HTML报告，每张图展示3种方案的mask/椭圆/矫正/读数。

运行: D:/minconda/envs/pytorch5060/python.exe exp_ellipse_mask_strategies.py
"""
...
def mask_hsv_edge_fusion(crop_bgr):
    """HSV + Canny边缘融合：先HSV锁定区域，再在区域内做边缘检测精修轮廓"""
    h, w = crop_bgr.shape[:2]
    hsv = cv2.cvtColor(crop_bgr, cv2.COLOR_BGR2HSV)
    _, S_, V_ = cv2.split(hsv)
    v_thr = np.percentile(V_, 55)
...
def mask_edge_direct(crop_bgr):
    ...
    if contours:
        ...
        cv2.drawContours(mask, [c], -1, 255, -1)  # 填充
...
def main():
    ...
    imgs = sorted([f for f in os.listdir(data_dir)
                   if f.lower().endswith((".jpg", ".png", ".jpeg"))])[:20]  # 只测前20张
    ...
    def make_patched_fit(strategy_fn):
        """生成一个用指定策略的_fit_face_ellipse替代函数"""
        def patched(crop_bgr):
            mask, _ = strategy_fn(crop_bgr)
            ellipse, comp, status = fit_ellipse_from_mask(mask, crop_bgr)
            if ellipse is None:
                # 返回mask供调试，椭圆为None触发回退
                return None, mask
            return ellipse, comp
        return patched

    cards = []
    # 统计：每种策略的读数Δ（相对off基线）和椭圆面积
    agg = {s.__name__: {"deltas": [], "areas": [], "fail": 0} for s in strategies}
    ...
    for i, name in enumerate(imgs):
        ...
        for strategy in strategies:
            ...
            if ellipse is not None:
                ...
                # 用这个策略的椭圆矫正后跑完整推理拿读数
                model._fit_face_ellipse = make_patched_fit(strategy)
                try:
                ...
                if o_res is not None and e_res is not None:
                    ...
                    agg[strategy.__name__]["deltas"].append(delta)
                    agg[strategy.__name__]["areas"].append(area_pct)
            else:
                rectified = None
                overlay = None
                ratio = None
        ...
        cards.append(
            ...
            f'<h3>#{i+1} {html.escape(name)} · off基线读数={o_txt}</h3>'
            f'<div class="grid">'
    ...
    # 汇总统计
    print("\n" + "=" * 70)
    print("汇总统计（读数Δ = |ellipse读数 - off基线读数|）")
    print("=" * 70)

    # 拆分正拍/斜拍统计（按HSV baseline的轴比判定）
    for s in strategies:
        st = agg[s.__name__]
        frontal_deltas = []  # 正拍（轴比≥0.85）
        tilted_deltas = []   # 斜拍（轴比<0.85）

        # 重跑一遍提取轴比分组（这里为了简化，直接在results里记录分组标志更好，但这样改动小）
        # 实际我们可以在循环里就记录，这里简化处理
        pass  # 暂时先输出全局统计
    ...
    for s in strategies:
        st = agg[s.__name__]
        if st["deltas"]:
            mean_d = sum(st["deltas"]) / len(st["deltas"])
            max_d = max(st["deltas"])
            mean_area = sum(st["areas"]) / len(st["areas"]) * 100
            print(f"{s.__name__:<25} N={len(st['deltas']):>3} fail={st['fail']:>2} "
                  f"meanΔ={mean_d:.4f} maxΔ={max_d:.4f} 椭圆占比={mean_area:.0f}%")
            summary_rows.append(
                f'<tr><td>{s.__name__}</td><td>{len(st["deltas"])}</td>'
                f'<td>{st["fail"]}</td><td>{mean_d:.4f}</td>'
                f'<td>{max_d:.4f}</td><td>{mean_area:.0f}%</td></tr>'
            )
        else:
            print(f"{s.__name__:<25} 无有效数据 fail={st['fail']}")
            summary_rows.append(
                f'<tr><td>{s.__name__}</td><td>0</td>'
                f'<td>{st["fail"]}</td><td>—</td><td>—</td><td>—</td></tr>'
            )

    summary_table = (
        '<table class="summary"><thead><tr>'
        '<th>策略</th><th>有效N</th><th>失败</th><th>平均Δ读数</th>'
        '<th>最大Δ读数</th><th>椭圆平均占比</th>'
        '</tr></thead><tbody>'
        + "".join(summary_rows) +
        '</tbody></table>'
    )
...
<body>
<h1>椭圆校正 Mask 生成策略对比实验</h1>
<div class="meta">对比 HSV Baseline / HSV+Edge Fusion / Edge Direct 三种方案的mask质量、椭圆拟合、矫正效果和最终读数。<br>
重点看：椭圆占比越大越可能抓到外圈金属壳；Δ读数越小越稳定。</div>
{summary_table}
{''.join(cards)}
</body></html>"""

    out_path = os.path.join(PROJ, "test_results", "ellipse_mask_comparison.html")
